...
DS.RESTAdapter = DS.Adapter.extend({
  ...
  */
  createRecord: function(store, type, record) {
    ...
    return serializer.serializeIntoHash(data, type, record, { includeId: true }).
      ...
      return self.ajax(self.buildURL(type.typeKey), "POST", { data: data });
    });
  },
  ...
  */
  updateRecord: function(store, type, record) {
    ...
    return serializer.serializeIntoHash(data, type, record).then(function() {
      var id = get(record, 'id');

      return self.ajax(self.buildURL(type.typeKey, id), "PUT", { data: data });
    });
  },
